...
class RandomPlayer():
    ...
    def evaluation(self, empty):
            ...
            for j in range(self.size):
                ...
                if self.board[i][j] != 0:
                    if i == 0 or j==0 or i==self.size-1 or j==self.size-1:
                        ...
                        if self.board[i][j] == self.piece_pointer:
                            our_edge_count += 1
                        else:
        ...
        our_qd = 0
        opponent_qd = 0
        our_piece_pointer = self.piece_pointer
        if our_piece_pointer == 1:
            opponent_piece_pointer = 2
        else:
            opponent_piece_pointer = 1
        ...
        #score = min(max(score2, -4), 4) + (-4)*score4 + 5 * score1 + score3
        # if empty == 0:
        if self.piece_pointer == 1:
            score -= 2.5
        else:
            score += 2.5
        ...
        if True:
            ...
            our_score = 2*our_liberty_count + (4)*our_euler_score + 3*our_piece_count + opponent_edge_count
            opponent_score = 8*opponent_liberty_count + 2 * opponent_euler_score+ 18*opponent_piece_count + 2*our_edge_count
            
        #white strategy
        # if self.piece_pointer == 2:
        #     our_score = 4*our_liberty_count + (5)*our_euler_score + 5 * our_piece_count + opponent_edge_count
        #     opponent_score = 3*opponent_liberty_count + 4 * opponent_euler_score+ 8 * opponent_piece_count + 2*our_edge_count
        
        score = our_score - opponent_score

        # self.visualize_board()
        # print(f"piece pointer = {self.piece_pointer}")
        # print(f"5*{our_liberty_count}+3*{our_euler_score}+4*{our_piece_count}+{opponent_edge_count}")
        # print(f"4*{opponent_liberty_count}+2*{opponent_euler_score}+10*{opponent_piece_count}+2*{our_edge_count}")
        # print(f"our: {our_score} \n opponent: {opponent_score}")
        # print(f"score: {score}")
        
        
        #score = 4*score2 + (4)*score4 + 7 * score1 + 3*score3 
        ...
        #     self.visualize_board()
        #if self.piece_type == 1: pdb.set_trace()
        return score
            ...
            for i in range(self.size):
                for j in range(self.size):
                    if self.valid_place_check(i, j, self.opponent_piece_type, test_check = True):
                        go_obj_before_move_min = deepcopy(self)
